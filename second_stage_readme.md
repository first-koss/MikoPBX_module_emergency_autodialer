...
Версия модуля после второго этапа зафиксирована как `0.2.0` в:

- `module.json`;
- `VERSION`;
- `CHANGELOG.md`.

Дальше версии ведутся по схеме `MAJOR.MINOR.PATCH`: новые этапы и функциональные возможности повышают `MINOR`, исправления повышают `PATCH`, несовместимые изменения модели данных или публичного поведения повышают `MAJOR`.
